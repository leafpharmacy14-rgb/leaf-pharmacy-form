...
function checkAge() {
  ...
  var disp = document.getElementById("age-display");
  disp.innerHTML = '<span style="display:inline-block;background:#e8f5e9;color:#2e7d32;font-size:13px;padding:4px 12px;border-radius:20px;">' + age + '</span>';
...
function buildMessage(data) {
  var msg = "リーフ薬局 問診票\n";
  msg += "名前：" + data.name + "\n";
  msg += "生年月日：" + data.birthday + "\n";
  msg += "区分：" + data.age_group + "\n";
  if (data.phone) { msg += "電話：" + data.phone + "\n"; }
  msg += "症状：" + (data.symptom 
 "未記入") + "\n";
  msg += "アレルギー：" + (data.allergy  "なし") + "\n";
  msg += "服用中の薬：" + (data.current_med 
 "なし");
  return msg;
}
...
headers: { "Content-Type": "application/json" },
  ...
  try {
    if (liff.isInClient()) {
      liff.sendMessages([{ type: "text", text: buildMessage(data) }]);
    }
  } catch(e) {}
